...
// void report( FILE *stream, mraa_aio_context adc_a0 ) {
...
// 	fprintf( stream, "%s %2.1f\n", time_buf, temperature );
// }

void report_shutdown( FILE *file ) {
	char time_buf[9];
	time_t now = time(0);
...
void report_cmd( FILE *file, char *cmd ) {
	fprintf( file, "%s", cmd );
}
...
char buffer[256];
	...
	// 			exit(0);
	// 		}
	// 		else if( strncmp( cmd, "STOP", STOP_LEN ) == 0 ) {					stop_fl = 1;
	// 			if(log_fl)
	// 				report_cmd(log, cmd);	
	// 		}
